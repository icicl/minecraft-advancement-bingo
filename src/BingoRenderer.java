...
public class BingoRenderer extends MapRenderer {
    ...
    private void render_square(MapCanvas canvas, int x, int y, BingoPlayer player) {
        ...
        byte[][] pixels = (byte[][]) MATERIALS_DISPLAY.get(
                this.plugin.getConfig().getString("advancements." + this.plugin.game.goal(x, y) + ".icon").split(":")[1]
        );
        if (pixels == null) {
            Bukkit
                    .getLogger()
                    .severe(
                            "No texture pixel map found for " +
                                    this.plugin.getConfig().getString("advancements." + this.plugin.game.goal(x, y) + ".icon")
                    );
            return;
        }
        int i0 =
